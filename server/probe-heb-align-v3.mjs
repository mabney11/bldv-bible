...
say('probe-heb-align v3 — + energic nun, morph-value normalisation, NT segmentation');
...
// SEGMENTATION. The NT edition writes multi-WORD units solid (𐤀𐤕𐤄𐤀𐤋𐤄𐤉𐤌 =
// 𐤀𐤕 + 𐤄𐤀𐤋𐤄𐤉𐤌, 𐤌𐤍𐤄𐤔𐤌𐤉𐤌 = 𐤌𐤍 + 𐤄𐤔𐤌𐤉𐤌) where the OT edition spaces them, so the
// whole-word key misses even though BOTH halves are attested. Split into a
// sequence of forms that ALREADY EXIST in the OT table — nothing is invented; a
// word with no fully-attested segmentation stays a miss. Fenced: at most
// MAX_SEG parts, each >= 2 letters. Segmentation ambiguity is counted so the
// inference is visible rather than folded into the headline.
const MAX_SEG = 3;
function segmentations(w, depth = MAX_SEG) {
    const chars = [...w];
    const out = [];
    const walk = (i, acc) => {
        if (out.length > 8) return;                       // enough to judge ambiguity
        if (i === chars.length) { if (acc.length >= 2) out.push([...acc]); return; }
        if (acc.length >= depth) return;
        for (let len = 2; i + len <= chars.length; len++) {
            const part = chars.slice(i, i + len).join('');
            if (!FORMS.has(part)) continue;
            acc.push(part); walk(i + len, acc); acc.pop();
        }
    };
    walk(0, []);
    // fewest parts first, then longest leading part
    return out.sort((a, b) => a.length - b.length || b[0].length - a[0].length);
}
...
for (const row of ntRows) {
    for (const w of verseWords(row)) {
        ...
        if (!form) {
            const segs = segmentations(w);
            if (segs.length) {
                ntSeg++; b.hit++;
                if (segs.length > 1) ntSegAmbig++;
                segSamples.length < 400 && segSamples.push({
                    form: w, parts: segs[0].join(' + '), ways: segs.length,
                    occurrences: 0,
                });
                continue;
            }
...
const ntHit = ntExact + ntMatres + ntSeg;
...
say(`    via segmentation into OT forms: ${ntSeg}  (${pct(ntSeg, ntHit)})   ← solid-written multi-word units`);
say(`      of those, >1 segmentation   : ${ntSegAmbig}  (${pct(ntSegAmbig, ntSeg)})   ← needs a tie-break rule`);
...
say('');
say('Segmented forms (each part is an attested OT form):');
const segAgg = new Map();
for (const r of segSamples) {
    if (!segAgg.has(r.form)) segAgg.set(r.form, { form: r.form, parts: r.parts, ways: r.ways, seen: 0 });
    segAgg.get(r.form).seen++;
}
table([...segAgg.values()].sort((a, b) => b.seen - a.seen).slice(0, SAMPLES),
      ['form', 'parts', 'ways', 'seen']);
