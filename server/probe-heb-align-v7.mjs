...
// server.js normalises BOTH the morph KEYS and VALUES because the DB stores the
// full-word forms ('unclassified_final=paragogic_nun'), not the short codes. v2
// omitted this, which is why uvf=N never fired and 𐤕𐤌𐤕𐤅𐤍 stayed unaligned.
...
const MORPH_VAL_NORM = {
    ...
    // unclassified_final — the paragogic/energic nun lives here
...
function morphAttrs(morph) {
    ...
    for (const seg of String(morph || '').split('|')) {
        ...
        if (eq < 1) continue;
        const rawK = seg.slice(0, eq).trim();
        const rawV = seg.slice(eq + 1).trim();
        if (!rawK || !rawV || rawV === 'absent' || rawV === 'none') continue;
        const k = MORPH_KEY_NORM[rawK] || rawK;
        const vNorm = rawV.replace(/_\([^)]*\)$/, '');          // drop _(annotation)
        out[k] = MORPH_VAL_NORM[rawV] || MORPH_VAL_NORM[vNorm] || vNorm.replace(/=+$/, '');
    }
    return out;
}
// ── TAIL RULES ──────────────────────────────────────────────────────────────
// Every rule below was READ OFF the v4 required-tail table, not proposed from
// general Hebrew morphology. Each returns Map(tail -> rule name) so the report
// can show what each rule actually earned; a rule that pays for nothing should
// be deleted rather than kept on faith.
//
//   prs-allomorph  poetic/alternate suffix spellings on tagged prs
//                  (𐤋𐤌𐤅 3mp=𐤌𐤅, 𐤋𐤄𐤅𐤍 3mp=𐤄𐤅𐤍, 𐤏𐤋𐤅𐤄𐤉 3ms=𐤄𐤉, 𐤅𐤋𐤊𐤄 2ms=𐤊𐤄)
//   paragogic-nun  verb + vt=impf + nu=pl. UNTAGGED in this corpus — uvf reads
//                  absent — which is why the v3 uvf=N path never fired.
//   he-directional nmpr/subs/advb/intj with no prs: 𐤌𐤑𐤓𐤉𐤌𐤄, 𐤄𐤄𐤓𐤄, 𐤌𐤋𐤌𐤏𐤋𐤄
//   he-cohortative verb at impv, or ps=p1 wayq/impf: 𐤄𐤁𐤄, 𐤅𐤀𐤔𐤋𐤇𐤄, 𐤅𐤄𐤁𐤉𐤀𐤄
//   he-3ms         st=c|prs=3ms written with he: 𐤀𐤄𐤋𐤄, 𐤊𐤎𐤅𐤕𐤄, 𐤁𐤄
//   particle-prs   prde/nega/advb/intj/prep/prps carrying a pronominal suffix
//                  that BHS does not tag at all (no prs key): 𐤄𐤍𐤍𐤉, 𐤀𐤉𐤍𐤍𐤅,
//                  𐤀𐤉𐤍𐤊, 𐤅𐤄𐤍𐤌. The v3 energic-nun rule was gated on a.prs, so
//                  it could not reach these.
// Every entry below is keyed to a prs TAG the corpus supplies, so adding forms
// does not loosen anything — a tail is only offered when the token is already
// tagged with that person/gender/number. Entries marked (v7) were read off the
// v6 residual table; several are Aramaic, which is why Daniel and Ezra sat at
// the bottom of the per-book alignment table from the very first run.
const PRS_ALLO = {
    '1cs': ['𐤉', '𐤍𐤉', '𐤄𐤉', '𐤃𐤉'],                      // 𐤏𐤌𐤃𐤉 (v7)
    '1cp': ['𐤍𐤅', '𐤍𐤀'],                                   // 𐤏𐤋𐤉𐤍𐤀 Aramaic (v7)
    '2ms': ['𐤊', '𐤊𐤄', '𐤍𐤊', '𐤅𐤊', '𐤊𐤌'],              // 𐤀𐤁𐤅𐤊, 𐤀𐤋𐤉𐤊𐤌 (v7)
    '2fs': ['𐤊', '𐤊𐤉'],                                      // 𐤍𐤔𐤉𐤊𐤉, 𐤋𐤊𐤉 (v7)
    '2mp': ['𐤊𐤌', '𐤊𐤅𐤍', '𐤊'],                            // 𐤀𐤋𐤄𐤊𐤅𐤍 Aramaic (v7)
    '2fp': ['𐤊𐤍', '𐤊𐤍𐤄'],                                  // 𐤊𐤆𐤌𐤕𐤊𐤍𐤄 (v7)
    '2cp': ['𐤊𐤌', '𐤊𐤅𐤍'],                                  // NEW TAG — 𐤍𐤐𐤔𐤕𐤉𐤊𐤌 (v7)
    '3ms': ['𐤅', '𐤄𐤅', '𐤍𐤅', '𐤄𐤉', '𐤅𐤄𐤉', '𐤄', '𐤌𐤅', '𐤍𐤄𐤅'],  // 𐤏𐤋𐤉𐤌𐤅 (v7)
    '3fs': ['𐤄', '𐤍𐤄', '𐤊'],                                // 𐤀𐤁𐤉𐤊 tagged 3fs (v7)
    '3mp': ['𐤌', '𐤄𐤌', '𐤍𐤌', '𐤌𐤅', '𐤄𐤅𐤍'],
    '3fp': ['𐤍', '𐤄𐤍', '𐤍𐤄', '𐤄𐤍𐤄'],                     // 𐤋𐤁𐤃𐤍𐤄, 𐤀𐤕𐤍𐤄 (v7)
};
...
/** Map(tail -> rule) the LAST token's morph could legitimately add.
 *
 *  v5 built this by repeatedly DOUBLING a working set, once per candidate
 *  suffix. With ~20 particle-prs candidates that is up to 2^20 tails per token,
 *  rebuilt at every position of every run — v5 could not finish a run. It was
 *  also wrong: alternatives WITHIN a category are mutually exclusive (a 3ms
 *  suffix is 𐤅 or 𐤄𐤅 or 𐤍𐤅, never concatenated), and the doubling happily
 *  produced 𐤉𐤌 + 𐤌 = 𐤉𐤌𐤌.
 *
 *  Correct shape: pick at most ONE alternative per category, then take the
 *  product ACROSS categories in surface order (ending, then pronominal suffix).
 *  Bounded at ~6 x ~21 entries. Memoised on pos+morph, which repeat heavily
 *  across 490k tokens, so the whole thing collapses to a Map lookup.
 */
const _tailCache = new Map();
function reconstructableTails(tok) {
    const cacheKey = (tok.pos || '') + '\u0000' + (tok.morph || '');
    ...
    // Category 1 — nominal/verbal ending. '' means the category contributes nothing.
    const endings = [['', null]];
    if (a.nme && NME_PALEO[a.nme]) for (const q of NME_PALEO[a.nme]) endings.push([q, 'nme']);
    if (a.vbe && VBE_PALEO[a.vbe]) for (const q of VBE_PALEO[a.vbe]) endings.push([q, 'vbe']);
    if (a.uvf && UVF_PALEO[a.uvf]) for (const q of UVF_PALEO[a.uvf]) endings.push([q, 'uvf']);
    // v6 caught only impf + plural. The residual table adds jussive (𐤉𐤏𐤔𐤅𐤍,
    // 𐤉𐤒𐤈𐤉𐤓𐤅𐤍), wayyiqtol (𐤅𐤉𐤓𐤉𐤁𐤅𐤍, 𐤅𐤕𐤒𐤓𐤁𐤅𐤍) and the 2fs imperfect
    // (𐤕𐤃𐤁𐤒𐤉𐤍, 𐤕𐤔𐤕𐤊𐤓𐤉𐤍). Still gated on verb + a specific tense/agreement —
    // NOT broadened to the perfect, where the table shows only 3 occurrences and
    // a trailing 𐤍 is far more likely to be a root radical.
    if (isVerb && ['impf', 'juss', 'wayq'].includes(a.vt) &&
        (a.nu === 'pl' || (a.ps === 'p2' && a.gn === 'f')))
        endings.push(['𐤍', 'paragogic-nun']);
    if (!a.prs && NOMINAL_POS.has(pos)) endings.push(['𐤄', 'he-directional']);
    if (isVerb && (a.vt === 'impv' || ((a.vt === 'wayq' || a.vt === 'impf') && a.ps === 'p1')))
        endings.push(['𐤄', 'he-cohortative']);

    // Category 2 — pronominal suffix. Tagged prs uses its own allomorphs; an
    // UNTAGGED particle may carry any of them (𐤄𐤍𐤍𐤉, 𐤀𐤉𐤍𐤍𐤅 — BHS tags no prs).
    const suffixes = [['', null]];
    if (a.prs && PRS_ALLO[a.prs]) for (const q of PRS_ALLO[a.prs]) suffixes.push([q, 'prs-allomorph']);
    else if (!a.prs && PARTICLE_POS.has(pos)) for (const q of ALL_PRS_FORMS) suffixes.push([q, 'particle-prs']);

    const out = new Map();
    for (const [e, eRule] of endings) {
        for (const [sfx, sRule] of suffixes) {
            const tail = e + sfx;
            if (!tail || out.has(tail)) continue;
            out.set(tail, sRule || eRule);
        }
    }
    _tailCache.set(cacheKey, out);
    return out;
}
...
say('probe-heb-align v7 — rare allomorphs from the v6 residual table (mostly Aramaic)');
...
function matchAt(tk, s, w) {
    ...
    for (let j = s; j < tk.length && run.length < MAXRUN; j++) {
        ...
        for (const [tail, rule] of reconstructableTails(run[run.length - 1])) {
            if (acc + tail === w) return { ok: true, tier: 'suffix', rule, end: j + 1, run: [...run] };
        }
...
function alignVerse(hw, tk) {
    ...
    for (let i = 0; i < hw.length; i++) {
        ...
        // DIAGNOSTIC: if some run starting at ti is a strict PREFIX of this word,
        // record the tail it would have needed and the morph of the token that
        // ought to supply it. This turns "add another suffix rule" from a guess
        // into a frequency table read off the corpus.
        let need = null;
        {
            let acc = '';
            for (let j = ti; j < tk.length && j < ti + MAXRUN; j++) {
                acc += tk[j].paleo;
                if (w.startsWith(acc) && acc.length < w.length) {
                    need = { tail: w.slice(acc.length), morph: tk[j].morph || '(none)',
                             pos: tk[j].pos || '', run_end: acc };
                } else if (!w.startsWith(acc)) break;
            }
        }
        res.push({ word: w, ok: false, tier: 'miss', start: ti, need,
...
const FORMS = new Map();              // heb surface -> Map(strongsSig -> {count, morphSigs:Set, canons:Set, morphemes})
// Concatenation of 2-3 CONSECUTIVE aligned OT words -> the parts, with a count.
// The NT edition writes some multi-word units solid; requiring the parts to occur
// ADJACENTLY in the OT is a far stronger test than "both parts exist somewhere",
// which is what let v3 pick 𐤊𐤋 + 𐤂𐤅 + 𐤐𐤊.
const SEQ = new Map();                // concat -> Map(partsKey -> {parts, count})
const perBook = new Map();
const offsetUsed = new Map();         // canon -> Map(offset -> chapters)
const missSamples = [];
let wTotal = 0, tierCount = { exact: 0, suffix: 0, matres: 0, miss: 0 }, vNoBhs = 0;
const tailNeed = new Map();      // "tail\0morph" -> {tail, morph, n, example}
const ruleHits = new Map();      // which reconstruction rule earned each alignment
...
let _chaptersDone = 0;
const _totalChapters = byChapter.size;
for (const [key, verses] of byChapter) {
    const canon = verses[0].canon_id;
    if (++_chaptersDone % 100 === 0)
        process.stderr.write(`\r  aligning… chapter ${_chaptersDone}/${_totalChapters}   `);
    // pick the offset that aligns the most words in this chapter — measured
    // Try the offsets in the given order and STOP as soon as one aligns almost
    // everything. Nearly every chapter fits at 0, so this removes ~2 of the 3
    // search passes over the whole OT.
    let best = null;
    for (const d of OFFSETS) {
        ...
        if (tot && ok / tot >= 0.95) break;          // good enough — don't search on
    }
    ...
    for (const row of verses) {
        ...
        for (const r of alignVerse(hw, tk)) {
            ...
            if (r.ok) verseAligned.push(r.word); else verseAligned.push(null);
            if (!r.ok) {
                if (r.need) {
                    const k = `${r.need.tail}\u0000${r.need.morph}`;
                    if (!tailNeed.has(k)) tailNeed.set(k, { tail: r.need.tail, morph: r.need.morph, n: 0, example: r.word });
                    tailNeed.get(k).n++;
                }
            ...
            b.aligned++;
            if (r.tier === 'suffix') { b.suffix++; ruleHits.set(r.rule, (ruleHits.get(r.rule) || 0) + 1); }
            ...
            if (!bucket.has(snSig)) bucket.set(snSig, { count: 0, morphSigs: new Set(), canons: new Set(), morphemes });
            const e = bucket.get(snSig); e.count++; e.morphSigs.add(mSig); e.canons.add(canon);
        }
        // adjacency sequences from this verse (only over consecutive ALIGNED words)
        for (let i = 0; i < verseAligned.length; i++) {
            if (!verseAligned[i]) continue;
            let concat = verseAligned[i];
            const parts = [verseAligned[i]];
            for (let k = 1; k <= 2 && i + k < verseAligned.length; k++) {
                const nxt = verseAligned[i + k];
                if (!nxt) break;
                concat += nxt; parts.push(nxt);
...
process.stderr.write(`\r  aligning… done (${_totalChapters} chapters)          \n`);
...
say('');
say('Which reconstruction rule earned each alignment (delete any that pays for nothing):');
table([...ruleHits.entries()].sort((a, b) => b[1] - a[1]).map(([rule, n]) => ({ rule, alignments: n })),
      ['rule', 'alignments']);
...
say('');
say('WHAT THE UNALIGNED WORDS NEEDED — tail the HEB word has that the BHS run lacks,');
say('paired with the morph of the token that should account for it. Each row is a');
say('candidate reconstruction rule, measured rather than guessed:');
table([...tailNeed.values()].sort((a, b) => b.n - a.n).slice(0, SAMPLES * 3)
        .map(r => ({ tail: r.tail, occurrences: r.n, example: r.example,
                     morph: r.morph.length > 46 ? r.morph.slice(0, 46) + '…' : r.morph })),
      ['tail', 'occurrences', 'example', 'morph']);
...
// Several top alternates look like Hebrew/Aramaic doublets (H3605/H3606,
// H3808/H3809). If a form's NON-dominant readings occur only in the Aramaic
// sections, the NT pick is not a judgement call at all. Measured, not assumed.
const ARAMAIC_CANONS = new Set([15, 27]);          // Ezra, Daniel
let confined = 0, confinedOcc = 0;
for (const [form, bucket] of FORMS) {
    const sigs = [...bucket.entries()].sort((a, b) => b[1].count - a[1].count);
    if (sigs.length < 2) continue;
    const alts = sigs.slice(1);
    if (alts.every(([, e]) => [...e.canons].every(c => ARAMAIC_CANONS.has(c)))) {
        confined++;
        confinedOcc += sigs.reduce((n, x) => n + x[1].count, 0);
    }
}
say(`  of those, alternates confined to Ezra/Daniel : ${confined}  (${pct(confined, ambiguous.length)} of ambiguous forms)`);
say(`  occurrences they cover                       : ${confinedOcc}`);
say('');
...
// SEGMENTATION, v4. Two tiers, both attestation-based, reported separately.
//
//  T1 ADJACENCY — the solid form equals a sequence of words that ACTUALLY OCCUR
//     next to each other in the OT (𐤀𐤕 𐤄𐤀𐤋𐤄𐤉𐤌). This is the strong test. v3
//     only required both parts to exist somewhere, which produced 𐤊𐤋 + 𐤂𐤅 + 𐤐𐤊.
//
//  T2 PROCLITIC — a single leading proclitic letter plus an attested form
//     (𐤔 + 𐤁𐤔𐤌𐤉𐤌). v3 could not reach these at all: it required every part to be
//     >= 2 letters, and Hebrew proclitics are one letter. The letter must be in
//     the proclitic set — the same set server.js uses — and the remainder must be
//     an attested whole-word form. Up to two stacked proclitics (𐤅𐤄, 𐤅𐤁).
//
// Anything neither tier resolves stays a miss. No scoring heuristics decide
// meaning here; attestation does.
const PROCLITIC_SET = new Set(['𐤅', '𐤄', '𐤁', '𐤋', '𐤊', '𐤌', '𐤔']);

function adjacencyMatch(w) {
    const m = SEQ.get(w);
    if (!m) return null;
    const opts = [...m.values()].sort((a, b) => b.count - a.count);
    return { parts: opts[0].parts, ways: opts.length, count: opts[0].count };
}
function procliticMatch(w) {
    const chars = [...w];
    for (let k = 1; k <= 2 && k < chars.length - 1; k++) {
        ...
        // EXACT attested stem only. v4 fell back to the mater index here, which
        // stacked two inferences and produced 𐤄𐤕𐤋𐤌𐤉𐤃𐤉𐤌 -> 𐤄 + 𐤕𐤋𐤌𐤃𐤌 ("the
        // disciples" read as he + a verb) and 𐤌𐤍𐤄𐤌𐤕𐤉𐤌 -> 𐤌 + 𐤅𐤍𐤄𐤌𐤕𐤌. One
        // inference per resolution: drop a proclitic OR fuzzy-match, never both.
        if (FORMS.has(stem)) return { parts: [...chars.slice(0, k), stem], ways: 1 };
    }
    return null;
}
...
let ntAdj = 0, ntAdjAmbig = 0, ntProc = 0;
...
for (const row of ntRows) {
    for (const w of verseWords(row)) {
        ...
        if (!form) {
            const adj = adjacencyMatch(w);
            const seg = adj || procliticMatch(w);
            if (seg) {
                b.hit++;
                if (adj) { ntAdj++; if (adj.ways > 1) ntAdjAmbig++; } else ntProc++;
                const key = w;
                if (!segAgg.has(key)) segAgg.set(key, {
                    form: w, parts: seg.parts.join(' + '), ways: seg.ways,
                    tier: adj ? 'adjacent' : 'proclitic', seen: 0 });
                segAgg.get(key).seen++;
                continue;
            }
...
const ntHit = ntExact + ntMatres + ntAdj + ntProc;
...
say(`    via OT ADJACENCY (attested seq): ${ntAdj}  (${pct(ntAdj, ntHit)})   ← solid-written multi-word units`);
say(`      of those, >1 attested split : ${ntAdjAmbig}  (${pct(ntAdjAmbig, ntAdj)})`);
say(`    via PROCLITIC + attested form : ${ntProc}  (${pct(ntProc, ntHit)})`);
say('');
say(`TRUSTWORTHY FLOOR (exact + plene only)  : ${ntExact + ntMatres}  (${pct(ntExact + ntMatres, ntWords)})`);
say(`  + adjacency                           : ${ntExact + ntMatres + ntAdj}  (${pct(ntExact + ntMatres + ntAdj, ntWords)})`);
say(`  + proclitic                           : ${ntHit}  (${pct(ntHit, ntWords)})`);
...
say('');
say('Segmented forms — check these by eye, they are the tier most likely to be wrong:');
table([...segAgg.values()].sort((a, b) => b.seen - a.seen).slice(0, SAMPLES * 2),
      ['form', 'parts', 'tier', 'ways', 'seen']);

say('');
// How much of the residue is a known particle glued to an UNKNOWN word? Those
// resolve for free once the NT-only vocabulary is tagged, so they size the
// curation job rather than representing a separate problem.
const LEAD_PARTICLES = ['𐤀𐤕', '𐤊𐤋', '𐤏𐤋', '𐤌𐤍', '𐤂𐤌', '𐤁𐤍', '𐤀𐤋', '𐤀𐤔𐤓'];
let missParticleLed = 0, missPlain = 0;
for (const [form, n] of missCount) {
    (LEAD_PARTICLES.some(p => form.startsWith(p) && [...form].length > [...p].length + 1)
        ? (missParticleLed += n) : (missPlain += n));
}
say(`unresolved led by a known particle (𐤀𐤕/𐤊𐤋/𐤏𐤋/𐤌𐤍/…) : ${missParticleLed}  (${pct(missParticleLed, missParticleLed + missPlain)})`);
say(`  -> these resolve once the trailing word is tagged; they are not a second problem`);
say(`unresolved standalone vocabulary                      : ${missPlain}`);
say('');
