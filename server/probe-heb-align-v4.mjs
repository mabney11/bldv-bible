...
function reconstructableTails(tok) {
    ...
    if (a.prs && PRS_PALEO[a.prs]) {
        // Three attested shapes for a suffix on a verb/particle:
        //   bare        𐤌𐤌 + 𐤅
        //   linking he  𐤎𐤅𐤎 + 𐤄𐤅
        //   ENERGIC NUN 𐤌𐤌 + 𐤍𐤅  (mimmennu, tochlenna, yahargeni) — v2 lacked this
        //   and it accounted for essentially the whole remaining OT 2%.
        const bare = PRS_PALEO[a.prs];
        for (const t of [...tails]) {
            tails.add(t + bare);
            tails.add(t + '𐤄' + bare);
            tails.add(t + '𐤍' + bare);
            tails.add(t + '𐤍');           // paragogic nun with no suffix consonant
        }
    }
    tails.delete('');
    return [...tails];
...
say('probe-heb-align v4 — + required-tail diagnostic, OT-adjacency segmentation');
...
function procliticMatch(w) {
    ...
    for (let k = 1; k <= 2 && k < chars.length - 1; k++) {
        ...
        const viaMatres = MATRES_INDEX.get(matresKey(stem));
        if (viaMatres && matresEquivalent(viaMatres, stem))
            return { parts: [...chars.slice(0, k), viaMatres], ways: 1, matres: true };
    }
    return null;
}
