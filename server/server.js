...
['lexicon.json', 'homographs.json', 'surface-strongs-overrides.json'].forEach(file => {
...
function hebNavIterate() {
    ...
    return surfDb.prepare(`
        SELECT o.book_id, o.word_raw, t.strongs
...
function hebOccForSN(sn, book = null) {
    ...
    const rows = _hebOccStmt.all(...books, navNormSN(sn));
...
function bhsVerseWords(book_id, chapter, verse, lexicon, homographs, surfaceOverrides) {
    ...
    if (navHebBooks().has(book_id)) {
        try {
            const rows = surfRowsFor(book_id, chapter, 'HEB').filter(r => r.verse === verse);
            if (rows.length) {
...
function findWordOccurrences(sn, match, book = null) {
    ...
    const occ = [];
    for (const v of verses) {
        const words = foldRowsToWords(rowStmt.all(v.book_id, v.chapter, v.verse));
