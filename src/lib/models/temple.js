 ...
 * (components/ModelScene.jsx) and the flat sheet (components/ModelSheet.jsx).
...
import { makeKit, V, SPEEDS, smooth, clamp01, timelineFor, BASE_MATERIALS } from './kit.js';
...
// ── Parts ────────────────────────────────────────────────────────────────────
// The toolkit (lib/models/kit.js) bound to the courts' ground: solids both
// renderers understand, idealized doorways and walls, slabs, stairs, furniture,
...
export const PIECES = [
  ...
  {
    // the figure itself is placed by the Dedicate story (model3d/templeExtras.js's buildDedication) inside this piece's group, so a click on him opens this card
...
// the fire, the days of the feast) key off these times — see templeExtras.js's DED.
...
const MARK_LABELS = {
  build: { 'gather-cedar': 'Labanawan', yasad: 'yasad', qayar: 'qayarawath', awalam: 'awalam', roof: 'gabayam', araz: 'araz', zahab: 'zahab', karawab: 'karawab', chatzar: 'chatzar', palace: 'malak', pillars: 'nachashath', vessels: 'manawarah', arawan: 'arawan' },
  dedicate: { qahal: 'qahal', nashaa: 'arawan', arawan: 'dabayar', singers: 'halal', inan: 'inan', shakan: 'shakan', kayawar: 'kayawar', thapalah: 'thapalah', ash: 'ash', kabawad: 'kabawad', zabach: 'zabach', chag: 'chag', shalach: 'shalach' },
  walk: { gate: 'gadawal', court: 'chatzar', altar: 'mazabach', yam: 'yam', bases: 'makanawath', pillars: 'imawadayam', porch: 'awalam', hayakal: 'hayakal', veil: 'parakath', dabayar: 'dabayar', karawab: 'karawab', arawan: 'arawan' },
};
...
// (pages/ModelPage.jsx, components/ModelScene.jsx, components/ModelSheet.jsx — the
// temple was the first model; Yachazaqaal's house, the mashakan and the cities
// describe themselves the same way.)
const CARD_INTRO = {
  dedicate: 'Play, and the bayath (house) is chanak (dedicated): the kahanayam (priests) bring the arawan (ark) from the gate to beneath the kanapay (wings) of the karawab (cherubim); the singers and the chatzatzarawath (trumpets) sound as achad (one); the inan (cloud) malaa (fills) the house so the priests cannot imad (stand); Shalamah (Solomon) kneels on the kayawar (scaffold) before the mazabach (altar) and spreads his hands; ash (fire) yarad (comes down) from shamayam (heavens); the kabawad (glory) fills the house and the people karai (bow) on the pavement; the sacrifices, the chag (feast) of seven days, and the people sent home shamach (glad).',
  roam: 'No story here: the house stands finished, and you walk it — through the gate of the gadawal (great) chatzar (court), around the yam (sea) and the makanawath (bases), between Yakayan (Jachin) and Baiz (Boaz); tap the dalathawath (doors) for their words, and tap them again to go in.',
  build: 'Play, and the house rises in the order the text gives it: the yasad (foundation), the qayarawath (walls) of aban (stone), araz (cedar) within, zahab (gold) over the araz (cedar), the karawab (cherubim), the doors, the chatzarawath (courts), the malak (king)\'s houses, Chayaram (Hiram)\'s nachashath (brass), and last the arawan (ark).',
  walk: 'Play, and walk in: through the gate of the gadawal (great) chatzar (court), past the mazabach (altar) and the yam (sea), between Yakayan (Jachin) and Baiz (Boaz), through the awalam (porch) and the dalathawath (doors), down the hayakal (temple) between the lampstands, through the parakath (veil) into the dabayar (oracle), beneath the kanapay (wings) of the karawab (cherubim).',
};
const CARD_EXTRAS = {
  dedicate: [{ heading: 'What the model adds', refs: DEDICATE_REFS, text: 'The text gives the order of the day and the kayawar (scaffold)\'s measure; the model supplies the rest and says so: the road the arawan (ark) takes (in through the east gates, up the steps, down the hayakal (temple)); four kahanayam (priests) at its poles; the singers in two blocks east of the mazabach (altar); the kayawar (scaffold) east of the altar facing the house; the inan (cloud) and the kabawad (glory) as light within the house; the ash (fire) as a shaft from shamayam (heavens); the qahal (assembly) as a crowd in the courts; and seven days as seven passes of the sun.' }],
  build: [{ heading: 'Where it all came from', refs: GATHER_REFS, text: 'The story opens on the land, drawn to a different scale (one map unit is a third of a km — the house is a dot on it). Chayaram (Hiram)\'s servants bring the araz (cedar) and barawash (fir) down from Labanawan (Lebanon) to the yam (sea) in rafts, to Yapaw (Joppa), and up to Yarawashalam (Jerusalem); the abanayam (stones) are cut in the har (mountain) and brought down; the nachashath (brass) is cast in the kakar (plain) of the Yaradan (Jordan) between Sakawath (Succoth) and Tzarathan (Zarethan) and carried up; the zahab (gold) comes out of Dawad (David)\'s store in his city — and the zahab (gold) of Parawayam (Parvaim), whose place the text never gives, comes in from the far side of the map, marked so.' }],
};
/** The branches the chooser offers, in the order the text tells them; `modes` are the model's modes each opens (the first is the page's). */
export const STORIES = [
  { key: 'build', label: 'Build', modes: ['build'], icon: 'build', go: 'Watch it rise →',
  ...
  { key: 'dedicate', label: 'Dedicate', modes: ['dedicate'], icon: 'dedicate', go: 'See the dedication →',
  ...
  { key: 'walk', label: 'Walk', modes: ['walk', 'roam'], icon: 'walk', go: 'Walk in →',
...
const IN_HOUSE = (p) => p.x > WEST_X - 1 && p.x < PORCH_X1 + 1 && Math.abs(p.z) < OUTER_Z + 1 && p.y < H.porchH;

export const MODEL = {
  id: 'temple', base: '/models/temple', defaultStory: 'walk',
  title: 'The Bayath (House) of Yahawah', paleo: '𐤁𐤉𐤕 𐤉𐤄𐤅𐤄',
  ...
  indexDescription: 'The bayath (house) Shalamah (Solomon) banah (built) for Yahawah as an interactive 3D model, measured in amah (cubits) from the text — watch it rise (1 Kings 6–7), see it dedicated (1 Kings 8), or walk in from the gate to the arawan (ark).',
  indexIntro: 'One model, measured in amah (cubits) from the text, and three ways through it. Each is its own story: choose one, and the model opens there — every part on it can be tapped for its measures and verses in any of them.',
  ground: H.courtY,
  ...
  stories: STORIES,
  card: {
    title: 'The bayath (house) Shalamah (Solomon) banah (built) for Yahawah',
    subtitle: (mode) => (mode === 'dedicate' ? '1 Kings 8 · 2 Chronicles 5–7' : '1 Kings 6–7 · 2 Chronicles 3–4'),
    intro: (mode) => CARD_INTRO[mode] || CARD_INTRO.walk,
    extras: (mode) => CARD_EXTRAS[mode] || [],
  ...
  strings: { parts: 'Parts of the house', storiesTitle: 'The house\'s stories', guidedTitle: 'Guided: from the gate to the ark, verse by verse' },
  roam: {
    eye: ROAM_EYE, start: ROAM_START, enter: ROAM_ENTER,
    openKeys: ['hayakal', 'dabayar'],                                            // the doors (hayakal) and the oracle's doors + veil (dabayar); the court gates add their own 'piece:i' keys
    openKey: (which) => (which === 'hayakal' ? 'hayakal' : 'dabayar'),            // a part's `open` ('hayakal' | 'oracle') → the walker's key
    openDefault: (k) => (k.includes(':') && !k.startsWith('great-court') ? 1 : 0),   // at the start the great court's gate — the outermost, where the walk begins — stands SHUT (fieldy), the inner court's open, the doors and the veil shut
  },
  scene: {
    whole: { id: 'house', group: 'house' },                                      // the piece that is the union of a group (no parts of its own): chosen, its group is haloed
    extras: 'temple',                                                            // the land the Build opens on and the dedication's actors (components/model3d/templeExtras.js)
    // Lamps inside the house (the lampstands' light — the hall is roofed and the sun cannot reach it), each burning by the story's progress
    lights: [
      { key: 'hallLight', color: 0xffd9a0, distance: 60, pos: [8, 12, 0], on: (P) => 420 * Math.max(0.3, P('manawarah')) * P('zahab') },
      { key: 'oracleLight', color: 0xffe2b0, distance: 40, pos: [-20, 12, 0], on: (P) => 300 * P('zahab') },
      { key: 'porchLight', color: 0xfff0d0, distance: 40, pos: [41, 12, 0], on: (P) => 160 * P('awalam') },
    ],
    walls: ['yasad', 'qayar', 'tzalai', 'awalam', 'roof', 'doors', 'oracle-doors', 'parakath', 'dabayar', 'chatzar', 'mazabach', 'great-court', 'yair', 'king-house', 'daughter-house', 'porch-pillars', 'porch-throne', 'kasaa'],
    notSolid: ['house', 'pillars', 'sharasharah', 'malak'],                    // the whole (a proxy group), the proxied pillars, the chains before the oracle (hung high), the king's figures of the dedication
    proxies: [-1, 1].map((sz) => ({ x: PILLAR.x, y: 0, z: sz * PILLAR.z, r: PILLAR.r + 0.3, h: PILLAR.h + PILLAR.capH + PILLAR.lilyH })),   // Yakayan and Baiz as plain cylinders: their four hundred pomegranates would cost more than the frame to ray against
    stairHouses: { yair: PALACE.forest, 'king-house': PALACE.king, 'daughter-house': PALACE.daughter },
    plan: {
      pieces: ['yasad', 'qayar', 'awalam', 'chatzar', 'great-court', 'mazabach', 'yair', 'king-house', 'daughter-house', 'porch-pillars', 'porch-throne', 'kasaa', 'makanawath'],
      extra: [{ x: SEA.x, z: SEA.z, r: SEA.r, y0: H.courtY, y1: H.courtY + 8 }, ...[-1, 1].map((sz) => ({ x: PILLAR.x, z: sz * PILLAR.z, r: PILLAR.r, y0: H.courtY, y1: 30 }))],   // the yam (sea), Yakayan and Baiz
      labels: [
      ...
      radius: 85,
      places: [
        { label: 'gate of the gadawal (great) chatzar (court)', bounds: { x0: 124, x1: 136, z0: -8, z1: 8 }, at: [134, 0] },
        { label: 'gate of the inner chatzar (court)', bounds: { x0: 106, x1: 116, z0: -6, z1: 6 }, at: [116, 0] },
        { label: 'kayawar (scaffold)', bounds: { x0: 90, x1: 100, z0: -6, z1: 6 }, at: [103, 0] },
        { label: 'mazabach (altar)', bounds: { x0: 66, x1: 86, z0: -10, z1: 34 }, at: [76, 38] },
        { label: 'yam (sea)', bounds: { x0: 52, x1: 64, z0: 26, z1: 38 }, at: [58, 42] },
        { label: 'makanawath (bases) · south', bounds: { x0: -40, x1: 24, z0: 30, z1: 36 }, at: [-8, 40] },
        { label: 'makanawath (bases) · north', bounds: { x0: -40, x1: 24, z0: -36, z1: -30 }, at: [-8, -40] },
        { label: 'bayath (house)', bounds: { x0: -47, x1: 54, z0: -27, z1: 27 }, children: [
          { label: 'steps, Yakayan and Baiz', bounds: { x0: 46, x1: 54, z0: -12, z1: 12 }, at: [56, 0] },
          { label: 'awalam (porch)', bounds: { x0: 36, x1: 46, z0: -10, z1: 10 }, at: [41, 0] },
          { label: 'hayakal (temple)', bounds: { x0: -10, x1: 36, z0: -10, z1: 10 }, at: [10, 0] },
          { label: 'manawarah (lampstands) and shalachanawath (tables)', bounds: { x0: -6, x1: 20, z0: -9, z1: 9 }, at: [8, 0] },
          { label: 'parakath (veil) and the gold altar', bounds: { x0: -10, x1: -4, z0: -8, z1: 8 }, at: [-3, 0] },
          { label: 'dabayar (oracle) — the karawab (cherubim) and the arawan (ark)', bounds: { x0: -30, x1: -10, z0: -10, z1: 10 }, at: [-15, 0] },
          { label: 'tzalai (side) rooms', bounds: { x0: -47, x1: 30, z0: -27, z1: -10 }, at: [0, -30] },
        ] },
        { label: 'awalam (porch) of imawadayam (pillars)', bounds: { x0: -5, x1: 45, z0: 63, z1: 100 }, at: [20, 85] },
        { label: 'awalam (porch) of the kasaa (throne)', bounds: { x0: 5, x1: 35, z0: 103, z1: 133 }, at: [20, 112] },
        { label: 'bayath (house) of the malak (king)', bounds: { x0: 45, x1: 115, z0: 103, z1: 165 }, at: [80, 134] },
        { label: 'bayath (house) for Paraih\'s banath (daughter)', bounds: { x0: -66, x1: -10, z0: 103, z1: 157 }, at: [-38, 130] },
        { label: 'bayath (house) of the yair (forest) of Labanawan', bounds: { x0: -125, x1: -75, z0: 65, z1: 165 }, at: [-100, 100] },
      ],
    },
    inHouse: IN_HOUSE,                                                           // the guided camera never pulls through a wall inside the house; out in the courts the walls are low and the eye is free
    floorAt: (c, mode, t) => (landAt(mode, t) > 0.5 ? -60 : (c.x > WEST_X - 2 && c.x < PORCH_X1 + 2 && Math.abs(c.z) < OUTER_Z + 2) ? 0 : H.courtY),   // never below the floor: the house's floor inside its footprint, the court's ground outside, the land's sea on the map
  },
  sheet: {
    plan: { x0: -131, x1: 131, z0: -79, z1: 175, scale: 2.3, left: 24, top: 26, grid: 20 },
    section: { x0: -48, x1: 62, y0: -6, y1: 122, scale: 2.75, left: 676, bottom: 64, tick: 20 },
    titles: ['PLAN · tzapawan (north) up · mazarach (east) right · one square = 20 amah', 'SECTION · through the middle, awalam to dabayar'],
    labels: [
    ...
    compass: [122, -66],
    aria: 'The house of Yahawah, drawn flat: a plan of the whole and a section through the house',
  },
};
