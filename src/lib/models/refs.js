...
const ALIASES = { 'song of songs': 'song of solomon', 'canticles': 'song of solomon', 'psalm': 'psalms', 'revelations': 'revelation' };
...
export function parseRefs(text) {
  ...
  for (const part of String(text || '').split(';')) {
    ...
    const m = /^\s*(?:([1-3]?\s?[A-Za-z][A-Za-z .']*?)\s+)?(\d+)(?::([\d\s,–\-]+))?\s*$/.exec(part);
    if (!m) continue;
