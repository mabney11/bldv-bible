        ...
        <section className="st-stagewrap">
          ...
          {roam ? (
            <div className="st-player tp-roambar">
              <span className="tp-roam-hint">{!use3d ? 'The plan and section show the finished house; switch to 3D to walk it.' : COARSE ? 'Left thumb on the view: a stick to walk · right thumb: drag to look · ⤒ jumps · tap a part for its details · tap a door again to go through it' : lock?.on ? 'The mouse is yours: move it to look, W A S D or the arrows to walk (Shift to run, space to jump), click what the crosshair is on for its details, a door twice to go through · the wheel pulls the view back to see yourself, forward again into your eyes · Esc gives the mouse back' : lock?.why ? `The browser would not hand over the mouse (${lock.why}) — drag the view to look instead · W A S D or the arrows to walk · click a part for its details` : 'Click the view once to take the mouse (nothing is chosen by that click) · then move it to look, W A S D or the arrows to walk, ← → turn, Shift to run, space to jump · click what the crosshair is on for its details, a door twice to go through · the wheel pulls the view back to see yourself · Esc gives the mouse back'}</span>
            </div>
          ) : (
